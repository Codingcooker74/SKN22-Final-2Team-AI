...
4. **대기 중인 추천(Next Queue) 안내**:
   - **대기 중인 추천 목록**이 있다면(없음이 아닌 경우), 반드시 답변 마지막에 다음 형식으로 질문을 추가하세요:
     "혹시 {다음_추천} 추천 상품도 바로 보여드릴까요?"
   - 예시: 다음_추천이 "바나나의 모래"이면 → "혹시 바나나의 모래 추천 상품도 바로 보여드릴까요?"
   - 대기 목록이 '없음'이면 이 질문을 하지 마세요.
